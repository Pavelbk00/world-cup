...
import { GOLDEN_BOOT_WINNER, MEDALISTS_RESULT, PLAYER_GOALS, PLAYOFF_RESULTS } from "./tournamentResults";
import type { MatchId, MatchResultState, PlayerScoreRow, PlayerState, Score } from "./types";
 ...
 */
export function pointsForMatch(actual: Score, pred: Score): { points: number; tier: PointTier } {
...
function computeGroupTopTwo(matches: MatchResultState[]): {
  ...
  const tables = new Map<string, Map<string, { team: string; points: number; gd: number; gf: number }>>();
  ...
  const thirdPlaceRows: Array<{ team: string; points: number; gd: number; gf: number }> = [];
  const qualifiedTeams = new Set<string>();
  for (const [group, rowsMap] of tables.entries()) {
    const rows = Array.from(rowsMap.values());
    rows.sort(
      (a, b) =>
        b.points - a.points || b.gd - a.gd || b.gf - a.gf || a.team.localeCompare(b.team, "ru")
    );
    if (rows.length >= 1) {
...
function scoreGroupStage(
  ...
  for (const prediction of player.groupStandings) {
    ...
    const predictedByPlace = [prediction.first, prediction.second, prediction.third, prediction.fourth];
...
function scoreTopScorer(player: PlayerState): number {
  ...
  if (!key) return 0;
  const goalEntry = Object.entries(PLAYER_GOALS).find(([name]) => normalizeName(name) === normalizeName(key));
  let points = (goalEntry?.[1] ?? 0) * 2;
  if (GOLDEN_BOOT_WINNER && normalizeName(key) === normalizeName(GOLDEN_BOOT_WINNER)) {
    points += 20;
  }
  return points;
}
...
function scoreMedalists(player: PlayerState): number {
  ...
  let points = 0;
  if (normalizeName(player.medalists.gold) === normalizeName(MEDALISTS_RESULT.gold)) points += 50;
  if (normalizeName(player.medalists.silver) === normalizeName(MEDALISTS_RESULT.silver)) points += 35;
  if (normalizeName(player.medalists.bronze) === normalizeName(MEDALISTS_RESULT.bronze)) points += 20;
  return points;
}
...
export function computeStandings(
  ...
  const { allGroupsFinished, placementsByGroup, qualifiedTeams } = computeGroupTopTwo(matches);
  for (const p of players) {
    ...
    total += groupStagePoints + playoffBonusPoints + topScorerPoints + medalistPoints;
